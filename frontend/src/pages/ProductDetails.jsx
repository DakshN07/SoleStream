...
const ProductDetails = () => {
  ...
  return (
    <div className="min-h-screen bg-surface md:h-screen md:overflow-hidden flex flex-col pt-20">
      <div className="flex-1 flex flex-col md:flex-row max-w-[100vw]">
        ...
        <div className="relative w-full md:w-2/3 h-[60vh] md:h-full bg-gray-50 flex-shrink-0">
          <button onClick={() => navigate(-1)} className="absolute top-6 left-6 z-10 flex items-center gap-2 font-medium text-sm hover:text-primary transition uppercase tracking-widest bg-white/80 px-4 py-2 rounded-full backdrop-blur-sm shadow-sm">
          ...
          {product.modelUrl ? (
            <>
              ...
              <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-10 text-xs font-semibold uppercase tracking-[0.2em] text-gray-500 bg-white/80 px-4 py-2 rounded-full backdrop-blur-sm">
                Drag to Rotate • Scroll to Zoom
              </div>
            </>
          ) : (
            <div className="w-full h-[60vh] md:h-full flex flex-col items-center justify-center bg-gray-100 p-6 md:p-12 relative">
               <div className="w-full max-w-lg aspect-square bg-white rounded-xl shadow-sm flex items-center justify-center overflow-hidden p-8 transition-all">
                  <img 
                  ...
                  <div className="flex gap-4 mt-8 shrink-0">
                    {product.images.map((img, idx) => (
                       <button 
                         ...
                         className={`w-16 h-16 rounded-lg bg-white border-2 overflow-hidden flex items-center justify-center ${activeImgIndex === idx ? 'border-primary shadow-md scale-110' : 'border-transparent opacity-60 hover:opacity-100 hover:scale-105'} transition-all`}
                       >
         ...
         {/* Right Details Panel */}
        <div className="w-full md:w-1/3 bg-white p-8 lg:p-12 overflow-y-auto no-scrollbar shadow-xl z-20 flex flex-col">
          <div className="flex-1 space-y-8">
            <div>
              <p className="text-primary font-bold uppercase tracking-widest text-sm mb-2">{product.brand}</p>
              <h1 className="text-4xl font-black tracking-tighter text-gray-900 leading-tight">{product.name}</h1>
                ...
                {product.colours.map(c => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-10 h-10 rounded-full border-2 transition-all ${selectedColor === hex ? 'border-primary scale-110 shadow-md ring-4 ring-primary/20' : 'border-gray-200'}`}
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                {product.sizes.map(s => (
                  <button
                    ...
                    className={`py-3 text-sm font-bold rounded-sm border transition-all ${selectedSize === s ? 'border-primary bg-primary text-white shadow-lg shadow-primary/30' : 'border-gray-200 text-gray-700 hover:border-gray-900'}`}
                  >
                    US {s}
                  </button>
                ))}
          ...
          <div className="mt-8 pt-8 border-t border-gray-100 flex flex-col gap-4 sticky bottom-0 bg-white lg:pb-0">
             ...
             <div className="flex gap-4">
               <button 
                 ...
                 className={`flex-1 flex items-center justify-center gap-3 py-4 font-bold uppercase tracking-widest text-sm rounded-sm transition-all ${liveStock === 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-gray-900 text-white hover:bg-primary shadow-xl hover:-translate-y-1'}`}
               >
               ...
               <button 
                 ...
                 className={`p-4 border rounded-sm transition-colors ${isFavorited ? 'text-red-500 border-red-500 bg-red-50' : 'border-gray-200 text-gray-400 hover:text-red-500 hover:border-red-500'}`}
               >
      ...
      {/* Reviews Section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 md:mt-24 border-t border-gray-100 pt-16">
         <div className="flex items-center gap-3 mb-10">
            <MessageSquare className="text-primary" />
            <h2 className="text-3xl font-bold tracking-tighter text-gray-900">Customer <span className="font-light italic text-gray-500">Reviews</span></h2>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
               ...
               {product.ratings && product.ratings.length === 0 ? (
                 ...
                 <div className="space-y-6">
                   {product.ratings?.map((r) => (
                     <div key={r._id} className="bg-gray-50 rounded-xl p-6 border border-gray-100">
                       <div className="flex justify-between items-start mb-4">
                         <strong className="text-gray-900 font-bold uppercase tracking-widest text-sm">{r.name}</strong>
                         <div className="flex">
            ...
            {/* Write Review Form */}
            <div>
               <h3 className="text-xl font-bold tracking-tighter text-gray-900 mb-6 uppercase">Write A Review</h3>
               
               {userInfo ? (
                  <form onSubmit={submitReviewHandler} className="bg-white rounded-xl shadow-xl shadow-gray-200/40 border border-gray-100 p-8 space-y-6">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-widest text-gray-900 mb-2">Rating</label>
                      <select 
                        value={rating} 
                        onChange={(e) => setRating(Number(e.target.value))}
                        className="w-full bg-gray-50 border border-gray-200 rounded p-4 text-sm focus:outline-primary focus:ring-1 focus:ring-primary"
                      >
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-widest text-gray-900 mb-2">Comment</label>
                      <textarea
                        rows="4"
                        value={comment}
                        ...
                        required
                        className="w-full bg-gray-50 border border-gray-200 rounded p-4 text-sm focus:outline-primary focus:ring-1 focus:ring-primary resize-none"
                      ></textarea>
                    </div>
